#encoding=utf-8
'''
@author: chenfei
@time: 2018-9-3 10:23:10
@description: rules for scrapy
'''

RULES = {
    '美食': ['美食', 'food', 'recipe', 'cate', 'cuisine', 'buffet', 'fast food', 'specialty', 'delicious', 'yummy'],
    '壁纸': ['壁纸', 'wallpaper', 'background image', ''],
    '咖啡': ['咖啡', 'coffee', 'espresso', 'cafe', 'latte', 'cappuccino', 'caffe mocha', 'macchiatto'],
    '汽车': ['汽车', 'car', 'vehicle',  'roadster', 'sportscar'],
    '设计': ['设计', 'design', 'designer', 'graphic design'],
    '植物': ['植物', 'plant', 'flora', 'flowers', 'plants'],
    '健身': ['健身', 'fitness', 'gym', 'bodybuilding', 'yoga', 'press', 'squat', 'work out'],
    '动物': ['动物', '宠物', 'animal', 'pet'],
    '型男': ['型男', '帅哥', '邓伦', '钟汉良', '胡歌', '林更新', 'handsome boy', 'handsome man', 'male star'],
}